...
#import <CICamera/UIView+Layout.h>
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            _tipLabel.frame = CGRectMake(8, 120, SCREENWIDTH - 16, 60);
            ...
            _settingBtn.frame = CGRectMake(0, 180, SCREENWIDTH, 44);
...
- (void)configDefaultSetting {
    ...
    CGFloat cropViewWH = MIN(SCREENWIDTH, SCREENHEIGHT) / 3 * 2;
    self.cropRect = CGRectMake((SCREENWIDTH - cropViewWH) / 2, (SCREENHEIGHT - cropViewWH) / 2, cropViewWH, cropViewWH);
}

- (void)configDefaultImageName {
    self.takePictureImageName = @"takePicture80_tz";
    self.photoSelImageName = @"photo_sel_photoPickerVc_tz";
    self.photoDefImageName = @"photo_def_photoPickerVc_tz";
    self.photoNumberIconImage = [self createImageWithColor:nil size:CGSizeMake(24, 24) radius:12]; // @"photo_number_icon";
    self.photoPreviewOriginDefImageName = @"preview_original_def_tz";
    self.photoOriginDefImageName = @"photo_original_def_tz";
    self.photoOriginSelImageName = @"photo_original_sel_tz";
}
...
- (void)configDefaultBtnTitle {
    self.doneBtnTitleStr = @"确定";// [NSBundle tz_localizedStringForKey:@"Done"];
    self.cancelBtnTitleStr = @"取消";// [NSBundle tz_localizedStringForKey:@"Cancel"];
    self.previewBtnTitleStr = @"预览";//[NSBundle tz_localizedStringForKey:@"Preview"];
    self.fullImageBtnTitleStr = @"原图";//[NSBundle tz_localizedStringForKey:@"Full image"];
    self.settingBtnTitleStr = @"设置";//[NSBundle tz_localizedStringForKey:@"Setting"];
    self.processHintStr = @"正在处理...";//[NSBundle tz_localizedStringForKey:@"Processing..."];
}
...
- (void)setCircleCropRadius:(NSInteger)circleCropRadius {
    ...
    self.cropRect = CGRectMake(SCREENWIDTH / 2 - circleCropRadius, SCREENHEIGHT / 2 - _circleCropRadius, _circleCropRadius * 2, _circleCropRadius * 2);
}
...
- (void)setCropRect:(CGRect)cropRect {
    ...
    _cropRectLandscape = CGRectMake((SCREENHEIGHT - widthHeight) / 2, cropRect.origin.x, widthHeight, widthHeight);
}

- (CGRect)cropRect {
    CGFloat screenHeight = [UIScreen mainScreen].bounds.size.height;
    BOOL isFullScreen = SCREENHEIGHT == screenHeight;
    if (isFullScreen) {
        return _cropRect;
    } else {
        CGRect newCropRect = _cropRect;
        newCropRect.origin.y -= ((screenHeight - SCREENHEIGHT) / 2);
        return newCropRect;
    }
}
...
- (void)viewDidLayoutSubviews {
    ...
    _progressHUD.frame = CGRectMake(0, progressHUDY, SCREENWIDTH, SCREENHEIGHT - progressHUDY);
    _HUDContainer.frame = CGRectMake((SCREENWIDTH - 120) / 2, (_progressHUD.tz_height - 90 - progressHUDY) / 2, 120, 90);
...
- (void)viewDidLayoutSubviews {
    ...
    BOOL isFullScreen = SCREENHEIGHT == [UIScreen mainScreen].bounds.size.height;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden && isFullScreen) top += [TZCommonTools tz_statusBarHeight];
        tableViewHeight = SCREENHEIGHT - top;
    } else {
        tableViewHeight = SCREENHEIGHT;
    }
    _tableView.frame = CGRectMake(0, top, SCREENWIDTH, tableViewHeight);
}
...
+ (UIImage *)tz_imageNamedFromMyBundle:(NSString *)name {
    NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[TZImagePickerController class]] pathForResource:@"TZImage" ofType:@"bundle"]];
    UIImage* image = [UIImage imageWithContentsOfFile:[bundle pathForResource:[NSString stringWithFormat:@"%@@2x",name] ofType:@"png"]];
    if (!image) {
        // 兼容业务方自己设置图片的方式
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    //[NSBundle bundleWithPath:[[NSBundle tz_imagePickerBundle] pathForResource:preferredLanguage ofType:@"lproj"]];
}
