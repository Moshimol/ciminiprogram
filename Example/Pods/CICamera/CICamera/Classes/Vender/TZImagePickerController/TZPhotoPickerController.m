...
- (void)configCollectionView {
    ...
    if (_showTakePhotoBtn) {
        _collectionView.contentSize = CGSizeMake(SCREENWIDTH, ((_model.count + self.columnNumber) / self.columnNumber) * SCREENWIDTH);
    } else {
        _collectionView.contentSize = CGSizeMake(SCREENWIDTH, ((_model.count + self.columnNumber - 1) / self.columnNumber) * SCREENWIDTH);
        if (_models.count == 0) {
...
- (void)viewDidLayoutSubviews {
    ...
    _collectionView.frame = CGRectMake(0, top, SCREENWIDTH, collectionViewHeight);
    _noDataLabel.frame = _collectionView.bounds;
    CGFloat itemWH = (SCREENWIDTH - (self.columnNumber + 1) * itemMargin) / self.columnNumber;
    ...
    _bottomToolBar.frame = CGRectMake(0, toolBarTop, SCREENWIDTH, toolBarHeight);
    ...
    _doneButton.frame = CGRectMake(SCREENWIDTH - _doneButton.tz_width - 12, 0, _doneButton.tz_width, 50);
    _numberImageView.frame = CGRectMake(_doneButton.tz_left - 24 - 5, 13, 24, 24);
    _numberLabel.frame = _numberImageView.frame;
    _divideLine.frame = CGRectMake(0, 0, SCREENWIDTH, 1);
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[TZImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && SCREENWIDTH <= 320) {
        return;
    }
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
