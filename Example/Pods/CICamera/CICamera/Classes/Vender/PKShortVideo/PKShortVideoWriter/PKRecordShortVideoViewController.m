...
- (void)viewDidLoad {
    ...
    NSBundle *imageBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[self class]] pathForResource: @"PKCamera" ofType: @"bundle"]];
    NSString *imagePath = [imageBundle pathForResource:@"PK_Camera_Turn@2x" ofType:@"png"];
    UIImage *image = [UIImage imageWithContentsOfFile:imagePath];
    UIBarButtonItem *transformItem = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStyleDone target:self action:@selector(swapCamera)];
...
- (void)recorder:(PKShortVideoRecorder *)recorder didFinishRecordingToOutputFilePath:(NSString *)outputFilePath error:(NSError *)error {
    ...
    if (error) {
        ...
        if (self.beginRecordTime != 0 && nowTime - self.beginRecordTime < self.videoMinimumDuration) {
            ...
            NSBundle *imageBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[self class]] pathForResource: @"PKCamera" ofType: @"bundle"]];
            NSString *playImagePath = [imageBundle pathForResource:@"PK_Play@2x" ofType:@"png"];
            UIImage *playImage = [UIImage imageWithContentsOfFile:playImagePath];
            ...
            NSString *refreshImagePath = [imageBundle pathForResource:@"PK_Delete@2x" ofType:@"png"];
            UIImage *refreshImage = [UIImage imageWithContentsOfFile:refreshImagePath];
