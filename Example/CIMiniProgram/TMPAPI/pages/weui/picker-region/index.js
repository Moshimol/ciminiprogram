...
//       return ['selector', 'multiSelector', 'time', 'date', 'region'].indexOf(value) !== -1
//     },
//       default: 'selector'
//   },
//   disabled: {
...
//             return (new RegExp(/^(20|21|22|23|[0-1]\d):[0-5]\d:[0-5]\d$/)).test(value)
//           } else {
//             return false
//           }
//         }
...
//               return (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])/)).test(value)
//             }
//             if (this.fields === 'day') {
//               return (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/)).test(value)
//             }
//             return false
//           } else {
//             return false
...
// },
data() {
  const originalYearData = Array.from(Array(2050), (v, k) => k + 1)
  const originalMonthData = Array.from(Array(12), (v, k) => k + 1).map(item => {
    return (Array(2).join(0) + item).slice(-2)
  })
  const originalDayData = Array.from(Array(31), (v, k) => k + 1).map(item => {
    return (Array(2).join(0) + item).slice(-2)
  })
  const originalHourData = Array.from(Array(24), (v, k) => k).map(item => {
    return (Array(2).join(0) + item).slice(-2)
  })
  const originalMinuteData = Array.from(Array(60), (v, k) => k).map(item => {
    return (Array(2).join(0) + item).slice(-2)
  })
  return {
    mode: 'region',
...
methods: {
  initLValue(value) {
    switch (this.mode) {
      ...
      case 'multiSelector': {
        this.lValue = value.map(item => item)
        break
      }
      case 'time': {
        let start = ['00', '00']
        let end = ['23', '59']
        if (this.start) {
          const s = (new RegExp(/^(20|21|22|23|[0-1]\d):([0-5]\d)$/)).exec(this.start)
          if (s && s.length) {
            s.shift()
            start = s
          }
        ...
        if (this.end) {
          const e = (new RegExp(/^(20|21|22|23|[0-1]\d):([0-5]\d)$/)).exec(this.end)
          if (e && e.length) {
            e.shift()
            end = e
          }
        }
        if (Number.parseInt(start[0]) * 60 + Number.parseInt(start[1]) > Number.parseInt(end[0]) * 60 + Number.parseInt(end[1])) {
          start = ['00', '00']
          end = ['23', '59']
        }
        let result = (new RegExp(/^(20|21|22|23|[0-1]\d):([0-5]\d)$/)).exec(value)
        ...
        const setTime = (tiemArray) => {
          let date = new Date()
          date.setFullYear(2000, 1, 1)
          date.setHours(...(tiemArray.map(item => Number.parseInt(item))))
          return date
        }
        this.startTime = start
        this.endTime = end
        let startDate = setTime(start)
        let currentDate = setTime(result)
        let endDate = setTime(end)
        if (startDate.getTime() <= currentDate.getTime() && currentDate.getTime() <= endDate.getTime()) {
        } else {
          if (startDate.getTime() > currentDate.getTime()) {
            result = JSON.parse(JSON.stringify(start))
          }
          if (currentDate.getTime() > endDate.getTime()) {
            result = JSON.parse(JSON.stringify(end))
          }
        }
        this.setTimeDisabled(result)
        this.lValue = result.map((item, index) => {
          if (index === 0) {
            return Number.parseInt(item) + 24
          }
          if (index === 1) {
            return Number.parseInt(item) + 60
          }
        })
        break
      }
      case 'date': {
        const length = this.fields === 'year' ? 1 : (this.fields === 'month' ? 2 : 3)
        let result
        if (result = (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/)).exec(value)) {
          result.shift()
          if (this.fields === 'year') {
            result = [Number.parseInt(result[0]) - 1]
          }
          if (this.fields === 'month') {
            result = [Number.parseInt(result[0]) - 1, Number.parseInt(result[1]) - 1 + 12]
          }
          if (this.fields === 'day') {
            result = [Number.parseInt(result[0]) - 1, Number.parseInt(result[1]) - 1 + 12, Number.parseInt(result[2]) - 1 + 31]
          }
        } else if (result = (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])$/)).exec(value)) {
          result.shift()
          if (this.fields === 'year') {
            result = [Number.parseInt(result[0]) - 1]
          }
          ...
          if (this.fields === 'day') {
            result = [Number.parseInt(result[0]) - 1, Number.parseInt(result[1]) - 1 + 12, 0 + 31]
          }
        } else if (result = (new RegExp(/^(\d{4})$/)).exec(value)) {
          result.shift()
          if (this.fields === 'year') {
            result = [Number.parseInt(result[0]) - 1]
          }
          if (this.fields === 'month') {
            result = [Number.parseInt(result[0]) - 1, 0 + 12]
          }
          if (this.fields === 'day') {
            result = [Number.parseInt(result[0]) - 1, 0 + 12, 0 + 31]
          }
        } else {
          if (this.fields === 'year') {
            result = [0]
        ...
        let end = ['9999', '12', '31']
        if (this.start) {
          const s1 = (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/)).exec(this.start)
          if (s1 && s1.length) {
            s1.shift()
            start = s1
          } else {
            const s2 = (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])$/)).exec(this.start)
            if (s2 && s2.length) {
              s2.shift()
              start = s2
            } else {
              const s3 = (new RegExp(/^(\d{4})$/)).exec(this.start)
              if (s3 && s3.length) {
                s3.shift()
                start = s3
              }
        ...
        if (this.end) {
          const e1 = (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/)).exec(this.end)
          if (e1 && e1.length) {
            e1.shift()
            end = e1
          } else {
            const e2 = (new RegExp(/^(\d{4})-(0[1-9]|1[0-2])$/)).exec(this.end)
            if (e2 && e2.length) {
              e2.shift()
              end = e2
            } else {
              const e3 = (new RegExp(/^(\d{4})$/)).exec(this.end)
              if (e3 && e3.length) {
                e3.shift()
                end = e3
              }
        ...
        startDate.setFullYear(...(start.map((item, index) => Number.parseInt(item) - (index === 1 ? 1 : 0))))
        let endDate = new Date()
        endDate.setFullYear(...(end.map((item, index) => Number.parseInt(item) - (index === 1 ? 1 : 0))))
        let currentDate = new Date()
        currentDate.setFullYear(...(result.map((item, index) => Number.parseInt(item) + (index === 0 ? 1 : (index === 1 ? 0 - 12 : 1 - 31)))))
        if (startDate.getTime() > endDate.getTime()) {
          start = ['0001', '01', '01'].slice(0, length)
          end = ['9999', '12', '31'].slice(0, length)
        }
        ...
        if (startDate.getTime() <= currentDate.getTime() && currentDate.getTime() <= endDate.getTime()) {
          ...
          result = JSON.parse(JSON.stringify(start)).slice(0, length)
        }
        let res = result.map(item => Number.parseInt(item))
        res[1] = res[1] % 12
        res[2] = res[2] % 31
        this.setDateDisabled(res)
        this.lValue = result.map((item, index) => {
          if (index === 0) {
            return Number.parseInt(item) - 1
          }
      ...
      case 'region': {
        let result = []
        if (value.length >= 1) {
          const index1 = this.regionData.findIndex(item => item.name === value[0])
          if (index1 > -1) {
            result.push(index1)
          } else {
            result = [0, 0, 0]
          ...
          if (value.length >= 2) {
            const index2 = this.regionData[index1].children.findIndex(item => item.name === value[1])
            if (index2 > -1) {
              result.push(index2)
            } else {
              result.push(0)
            ...
            if (value.length >= 3) {
              const index3 = this.regionData[index1].children[index2].children.findIndex(item => item.name === value[2])
              if (index3 > -1) {
                result.push(index3)
              } else {
                result.push(0)
  ...
  setTimeDisabled(current) {
    const real = current.map((item, index) => {
      if (index === 0) {
        return Number.parseInt(item) % 24
      }
      if (index === 1) {
    ...
    if (Number.parseInt(real[0]) !== Number.parseInt(this.startTime[0]) && Number.parseInt(real[0]) !== Number.parseInt(this.endTime[0])) {
      this.minuteDataDisabled = this.minuteData.map(item => false)
    } else if (Number.parseInt(real[0]) === Number.parseInt(this.startTime[0]) && Number.parseInt(real[0]) === Number.parseInt(this.endTime[0])) {
      this.minuteDataDisabled = this.minuteData.map(item => {
        const i = Number.parseInt(item)
        return i < Number.parseInt(this.startTime[1]) || i > Number.parseInt(this.endTime[1])
      })
    } else if (Number.parseInt(real[0]) === Number.parseInt(this.startTime[0])) {
      this.minuteDataDisabled = this.minuteData.map(item => {
        const i = Number.parseInt(item)
        return i < Number.parseInt(this.startTime[1])
      })
    } else if (Number.parseInt(real[0]) === Number.parseInt(this.endTime[0])) {
      this.minuteDataDisabled = this.minuteData.map(item => {
        const i = Number.parseInt(item)
        return i > Number.parseInt(this.endTime[1])
      })
    }
    this.hourDataDisabled = this.hourData.map(item => Number.parseInt(item) < Number.parseInt(this.startTime[0]) || Number.parseInt(item) > Number.parseInt(this.endTime[0]))
  },
  setDateDisabled(current) {
    this.yearDataDisabled = this.yearData.map(item => Number.parseInt(item) < Number.parseInt(this.startDate[0]) || Number.parseInt(item) > Number.parseInt(this.endDate[0]))
    if (this.fields === 'month' || this.fields === 'day') {
      this.monthDataDisabled = this.monthData.map((item, index) => {
        let s = new Date('2020')
        s.setFullYear(Number.parseInt(this.startDate[0]), (Number.parseInt(this.startDate[1]) % 12 || 12) - 1)
        let e = new Date('2020')
        e.setFullYear(Number.parseInt(this.endDate[0]), (Number.parseInt(this.endDate[1]) % 12 || 12) - 1)
        let c = new Date('2020')
        c.setFullYear(Number.parseInt(current[0]), ((Number.parseInt(item)) % 12 || 12) - 1)
        return s.getTime() > c.getTime() || c.getTime() > e.getTime()
      })
    }
    if (this.fields === 'day') {
      this.dayDataDisabled = this.dayData.map((item, index) => {
        let a = new Date()
        a.setFullYear(Number.parseInt(current[0]), (Number.parseInt(current[1] % 12) || 12) - 1, 0)
        if (Number.parseInt(item) > a.getDate()) {
          return true
        }
        let s = new Date('2020')
        s.setFullYear(Number.parseInt(this.startDate[0]), (Number.parseInt(this.startDate[1]) % 12 || 12) - 1, Number.parseInt(this.startDate[2]) % 31 || 31)
        let e = new Date('2020')
        e.setFullYear(Number.parseInt(this.endDate[0]), (Number.parseInt(this.endDate[1]) % 12 || 12) - 1, Number.parseInt(this.endDate[2]) % 31 || 31)
        let c = new Date('2020')
        c.setFullYear(Number.parseInt(current[0]), (Number.parseInt(current[1]) % 12 || 12) - 1, Number.parseInt(item) % 31 || 31)
        return s.getTime() > c.getTime() || c.getTime() > e.getTime()
      })
    }
  },
  rectifyValue(current, min, max, index, offset = 0, dividend) {
    if (((current[index] + offset) % dividend || 31) < Number.parseInt(min[index])) {
      current[index] = Number.parseInt(min[index]) - offset
    }
    if (((current[index] + offset) % dividend || 31) > Number.parseInt(max[index])) {
      current[index] = Number.parseInt(max[index]) - offset
    }
  }
},
render(h) {
    ...
    switch (this.mode) {
        ...
        pickerView = h('x-picker-view', {
          ...
          on: {
            change: (e) => {
              ...
              const changeIndex = this.lValue.findIndex((item, index) => item !== newValue[index])
              if (changeIndex !== -1) {
                this.lValue = newValue
              }
            }
          },
          props: {
          ...
          h('x-picker-view-column', {
            class: [
              'picker_view_column_height'
            ],
          }, this.range.map(item => {
            return h('div', {
              class: [
                'picker_view_column_item'
              ]
            }, typeof item === 'object' ? item[this.rangeKey] : item)
          }))
        ])
        break
      }
      case 'multiSelector': {
        pickerView = h('x-picker-view', {
          ...
          this.range.map(group => {
            return h('x-picker-view-column', {
              class: [
                'picker_view_column_height',
                'flex'
              ]
      ...
      case 'time': {
        pickerView = h('x-picker-view', {
          on: {
            change: (e) => {
              if (this.lValue[0] === 0 || this.lValue[0]) {
                if (this.lValue[0] < 24) {
                  this.hourData.unshift(...this.originalHourData.map(item => item))
                }
                if (this.hourData.length - this.lValue[0] < 24) {
                  this.hourData.push(...this.originalHourData.map(item => item))
                }
              }
              if (this.lValue[1] === 0 || this.lValue[1]) {
                if (this.lValue[1] < 60) {
                  this.minuteData.unshift(...this.originalMinuteData.map(item => item))
                }
                if (this.minuteData.length - this.lValue[1] < 60) {
                  this.minuteData.push(...this.originalMinuteData.map(item => item))
                }
              }
              const newValue = e.detail.value
              const changeIndex = this.lValue.findIndex((item, index) => item !== newValue[index])
              ...
              if (changeIndex === 0) {
                if (this.hourDataDisabled[newValue[0]]) {
                  newValue[0] = this.hourDataDisabled.findIndex(item => !item) + 24
                  // this.rectifyValue(newValue, this.startTime, this.endTime, 0, 24)
                }
                this.setTimeDisabled(format(newValue))
                if (this.minuteDataDisabled[newValue[1]]) {
                  newValue[1] = this.minuteDataDisabled.findIndex(item => !item) + 60
                  // this.rectifyValue(newValue, this.startTime, this.endTime, 1, 60)
                }
                this.setTimeDisabled(format(newValue))
                this.lValue = newValue
          ...
          }, this.hourData.map((item, index) => h('div', {
            class: [
              'picker_view_column_item'
            ],
            style: {
              color: this.hourDataDisabled[index] ? disabledColor : 'black'
            }
          }, item))),
          h('x-picker-view-column', {
            class: [
          ...
          }, this.minuteData.map((item, index) => h('div', {
            class: [
              'picker_view_column_item'
            ],
            style: {
              color: this.minuteDataDisabled[index] ? disabledColor : 'black'
            }
          }, item)))
        ])
        break
      ...
      case 'date': {
        pickerView = h('x-picker-view', {
          on: {
            change: (e) => {
              if (this.yearData.length - this.lValue[0] < 30) {
                this.yearData.push(...Array.from(Array(100), (v, k) => k + this.yearData.length))
              }
              if (this.lValue[1] === 0 || this.lValue[1]) {
                if (this.lValue[1] < 12) {
                  this.monthData.unshift(...this.originalMonthData.map(item => item))
                }
                if (this.monthData.length - this.lValue[1] < 12) {
                  this.monthData.push(...this.originalMonthData.map(item => item))
                }
              }
              if (this.lValue[2] === 0 || this.lValue[2]) {
                if (this.lValue[2] < 31) {
                  this.dayData.unshift(...this.originalDayData.map(item => item))
                }
                if (this.dayData.length - this.lValue[2] < 31) {
                  this.dayData.push(...this.originalDayData.map(item => item))
                }
              }
              ...
              let format = (arr) => {
                return arr.map((item, index) => {
                  ...
                  if (index === 1) {
                    return (Number.parseInt(item) + 1) % 12 || 12
                  }
                  if (index === 2) {
                    return (Number.parseInt(item) + 1) % 31 || 31
                  }
                })
              }
              if (changeIndex === 0) {
                if (this.yearDataDisabled[newValue[0]]) {
                  newValue[0] = this.yearDataDisabled.findIndex(item => !item)
                  // this.rectifyValue(newValue, this.startDate, this.endDate, 0, 1, 10000)
                }
                this.setDateDisabled(format(newValue))
                if (this.monthDataDisabled[newValue[1]]) {
                  newValue[1] = this.monthDataDisabled.findIndex(item => !item) + 12
                  // this.rectifyValue(newValue, this.startDate, this.endDate, 1, 1, 12)
                }
                this.setDateDisabled(format(newValue))
                if (this.dayDataDisabled[newValue[2]]) {
                  newValue[2] = this.dayDataDisabled.findIndex(item => !item) + 31
                  // this.rectifyValue(newValue, this.startDate, this.endDate, 2, 1, 31)
                }
                this.setDateDisabled(format(newValue))
                this.lValue = newValue
              } else if (changeIndex === 1) {
          ...
          h('x-picker-view-column', {
            ...
            const real = h('blank-div', {
              class: [
                'picker_view_column_item'
              ],
              style: {
                color: this.yearDataDisabled[index] ? disabledColor : 'black'
              }
            }, item)
            const blank = h('blank-div', {
              class: [
                'picker_view_column_item'
              ]
            })
            return this.lValue[0] - index < 50 || this.lValue[0] > -50 ? real : blank
          })),
          (this.fields === 'month' || this.fields === 'day') && h('x-picker-view-column', {
            class: [
              'picker_view_column_height',
              'flex'
            ]
          }, this.monthData.map((item, index) => h('div', {
            class: [
              'picker_view_column_item'
            ],
            style: {
              color: this.monthDataDisabled[index] ? disabledColor : 'black'
            }
          }, item))),
          (this.fields === 'day') && h('x-picker-view-column', {
            class: [
              'picker_view_column_height',
              'flex'
            ]
          }, this.dayData.map((item, index) => h('div', {
            class: [
              'picker_view_column_item'
            ],
            style: {
              color: this.dayDataDisabled[index] ? disabledColor : 'black'
            }
          }, item)))
        ])
        break
      ...
      case 'region': {
        pickerView = h('x-picker-view', {
          ...
          }, this.regionData.map(item => h('div', {
            class: [
              'picker_view_column_item_text'
            ],
            attrs: {
              align: 'center'
            }
          ...
          }, this.regionData[this.lValue[0]].children.map(item => h('div', {
            class: [
              'picker_view_column_item_text'
            ],
            attrs: {
          ...
          }, this.regionData[this.lValue[0]].children[this.lValue[1]].children.map(item => h('div', {
            class: [
              'picker_view_column_item_text'
            ],
            attrs: {
  ...
  const picker = h('wx-picker', {
    ...
    h('div', {
      ...
      style: [
        {
          display: this.showDelay || this.show ? 'block' : 'none'
        }
      ],
      class: [
        'picker_wrapper'
      ...
      h('div', {
        class: [
          'picker_content',
          this.show ? 'picker_content_in' : 'picker_content_out'
        ],
        on: {
          click: (e) => {
            e.stopPropagation()
        ...
        h('div', {
          class: [
            'picker_content_title'
          ]
        }, [
          h('div', {
            class: [
              'picker_content_title_btn',
              'picker_content_title_btn_cancel'
            ],
            style: {
          ...
          h('div', {
            ...
            on: {
              click: (e) => {
                let result
                const value = JSON.parse(JSON.stringify(this.lValue))
                switch (this.mode) {
                  case 'selector': {
                    result = {
                      value: value[0]
                  ...
                  case 'time': {
                    result = {
                      value: `${(Array(2).join(0) + value[0]).slice(-2)}:${(Array(2).join(0) + value[1]).slice(-2)}`
                    }
                    break
                  }
                  case 'date': {
                    result = `${(Array(4).join(0) + (value[0] + 1)).slice(-4)}`
                    if (this.fields === 'month') {
                      result += `-${(Array(2).join(0) + (((value[1] + 1) % 12) || 12)).slice(-2)}`
                    }
                    if (this.fields === 'day') {
                      result += `-${(Array(2).join(0) + (((value[1] + 1) % 12) || 12)).slice(-2)}-${(Array(2).join(0) + (((value[2] + 1) % 31) || 31)).slice(-2)}`
                    }
                    result = {
                      value: result
                    }
                  ...
                  case 'region': {
                    ...
                    result.code.push((this.regionData[value[0]].code + '000000').substr(0, 6))
                    result.value.push(this.regionData[value[0]].name)
                    result.code.push((this.regionData[value[0]].children[value[1]].code + '000000').substr(0, 6))
                    result.value.push(this.regionData[value[0]].children[value[1]].name)
                    result.code.push(this.regionData[value[0]].children[value[1]].children[value[2]].code)
                    result.value.push(this.regionData[value[0]].children[value[1]].children[value[2]].name)
                    break
                  }
                }
                this.$emit('change', {
...
computed: {
  regionData() {
    if (!!this.customItem) {
      const arr = this.originalRegionData.map(first => {
        first.children.map(second => {
          second.children.unshift({
            code: '',
      ...
      return JSON.parse(JSON.stringify(this.originalRegionData))
    }
  }
}}
