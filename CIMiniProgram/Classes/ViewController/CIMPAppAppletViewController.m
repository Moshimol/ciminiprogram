...
@interface HomeTitleLoading: UIView

@property (nonatomic,strong) NSArray<UIColor *>*colors;

@end

@implementation HomeTitleLoading
...
@interface HomeTitleLoadingView: UIView

@property (nonatomic,strong) UILabel *titleLabel;
@property (nonatomic,strong) UIImageView *iconView;
@property (nonatomic,strong) HomeTitleLoading *loadingView;
...
@implementation HomeTitleLoadingView {
...
- (HomeTitleLoading *)loadingView {
    if(!_loadingView) {
        _loadingView = [[HomeTitleLoading alloc] init];
...
@interface CIMPAppAppletViewController ()

@property (nonatomic,copy) NSString *mp_applet_page_id;
@property (nonatomic,strong) HomeTitleLoadingView *titleLoaddingView;
@property (nonatomic, strong) UIButton *leftButton;

@end

@implementation CIMPAppAppletViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.view.backgroundColor = UIColor.whiteColor;
    self.mp_applet_page_id = @"Mini_Program_Applet_Page";
    if (!self.navigationController.isNavigationBarHidden) {
        [self.navigationController setNavigationBarHidden:YES animated:YES];
    }
    
    _titleLoaddingView = [[HomeTitleLoadingView alloc] initWithFrame:CGRectMake(0, [UIApplication sharedApplication].statusBarFrame.size.height, self.view.bounds.size.width, 120)];
...
- (UIButton *)leftButton {
    if(!_leftButton) {
        ...
        [_leftButton setImage:[UIImage imageNamed:@"Menu_Back_Dark"] forState:UIControlStateNormal];
